...
- 抽 `Effects/Feedback`：当前已把爆炸、屏幕震动、命中特效和飘字抽到 `src/games/arcade/effects.js`，并把受击闪烁、短暂停顿/慢动作、命中高亮抽到 `src/games/arcade/feedback.js`，迁移到五个 Canvas 游戏；Boss 出场演出也已收敛到 `src/games/arcade/bosses.js`。
- 补 `SDK 文档/模板`：新增 `docs/complex-game-sdk.md` 和 `docs/templates/arcade-game-template.js`，用于后续复杂游戏快速接入公共循环、反馈、关卡、Boss、奖励和续玩能力。
